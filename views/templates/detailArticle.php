    ...
    <?php 
        if (empty($comments)) {
            ...
            foreach ($comments as $comment) {
                ...
                // Si on est connecté, on affiche le bouton de supression : 
                if (isset($_SESSION['user'])) 
                    {
                    echo '<div><a class="submit" href="index.php?action=deleteComment&id=' . $comment->getId() . '" ' . Utils::askConfirmation("Êtes-vous sûr de vouloir supprimer ce commentaire ?") . '>Supprimer</a></div>';
                    }
            }               
            echo '</ul>';
        } 
